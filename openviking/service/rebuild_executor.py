...
_rebuild_executor: "RebuildExecutor | None" = None


def get_rebuild_executor() -> "RebuildExecutor":
    global _rebuild_executor
    if _rebuild_executor is None:
        _rebuild_executor = RebuildExecutor()
...
class RebuildExecutor:
        ...
        *,
        ...
        async with LockContext(get_lock_manager(), [path], lock_mode="subtree"):
            if object_type == "global_namespace":
                await self._rebuild_global_namespace(
                    uri=uri,
                    mode=mode,
                    counters=counters,
                    ctx=ctx,
                ...
                await self._rebuild_agent_namespace(
                    uri=uri,
                    mode=mode,
                    counters=counters,
                    ctx=ctx,
                ...
                await self._rebuild_user_namespace(
                    uri=uri,
                    mode=mode,
                    counters=counters,
                    ctx=ctx,
    ...
    async def _rebuild_resource_vectors(
        ...
        *,
        ...
        await self._rebuild_resource_vectors_from_entries(
            root_uri=uri,
    ...
    async def _rebuild_resource_vectors_from_entries(
        self,
        *,
        root_uri: str,
    ...
    async def _rebuild_user_namespace(
        self,
        *,
        uri: str,
        mode: str,
        ...
        if mode == "semantic_and_vectors":
            await self._run_semantic_processor(uri=target_root, context_type="resource", ctx=ctx)
    ...
    async def _rebuild_agent_namespace(
        self,
        *,
        uri: str,
        mode: str,
        ...
        for skill_root in sorted(set(skill_roots)):
            ...
            await self._rebuild_skill(uri=skill_root, mode=skill_mode, counters=counters, ctx=ctx)
    ...
    async def _rebuild_global_namespace(
        self,
        *,
        uri: str,
        mode: str,
        ...
        *,
        ...
        if await viking_fs.exists(uri, ctx=ctx):
            ...
            if stat.get("isDir", stat.get("is_dir")):
                ...
                await self._rebuild_memory_directory_chain(
    ...
    async def _rebuild_memory_directory_chain(
        self,
        *,
