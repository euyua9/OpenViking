...
Rebuild semantic and/or vector artifacts for existing content already stored in OpenViking. This is an operational maintenance API intended for scenarios such as embedding model changes, VLM changes, vector store rebuild, or post-upgrade repair of existing indexes.
...
| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| uri | str | Yes | - | Viking URI to rebuild |
| mode | str | No | `vectors_only` | Rebuild mode: `vectors_only` or `semantic_and_vectors` |
| wait | bool | No | `true` | Whether to wait for completion |
| reason | str | No | None | Optional operation reason |
...
`viking://session/...` is not supported by `rebuild()`.
...
result = client.rebuild(
    ...
    mode="vectors_only",
    wait=True,
    reason="Rebuild vectors after embedding model change",
)
print(result)
```
...
```
POST /api/v1/content/rebuild
```

```bash
curl -X POST http://localhost:1933/api/v1/content/rebuild \
  -H "Content-Type: application/json" \
  -H "X-API-Key: your-key" \
  -d '{
    "uri": "viking://resources",
...
```bash
openviking rebuild viking://resources --mode vectors_only
```

```bash
openviking rebuild viking://agent/default/skills --mode semantic_and_vectors --wait false
```
...
- Rebuild is non-destructive. It uses rebuild/upsert behavior and does not require dropping the vector collection first.
- `viking://` rebuild fans out to supported top-level namespaces and excludes `session`.
- Namespace rebuilds such as `viking://user` or `viking://agent/default` propagate to their supported child content types.
...
- Rebuild uses the best currently recoverable source inputs. It is not guaranteed to replay the exact historical embedding input byte-for-byte in every case.
- Memory semantic rebuild is based on the currently persisted memory tree. It does not reconstruct the original chronological memory-extraction pipeline.
